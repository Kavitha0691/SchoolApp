...
const SearchInput = () =>{
    ...
    {data && 
         <div onClick={() => setUserInput('')} className="border absolute bg-white p-2 rounded-xl ml-14">
            {data.map(({title, slug} )  => <Link className="block mb-2" key={slug} href= {`/${slug}`}> {title}</Link>)}
        </div>
    }
        </div>
    )
